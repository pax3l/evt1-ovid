<?xml version="1.0" encoding="UTF-8"?>
                  <xsl:stylesheet xpath-default-namespace="http://www.tei-c.org/ns/1.0"
                    ...
                    exclude-result-prefixes="#all" ><xd:doc type="stylesheet">
                    <xd:short>
                  ...
                  </xd:doc><!-- This file was auto generated using EVT 1 Config Generator Tool http://evt.labcd.unipi.it/evt1-config --><!-- GLOBAL -->
                  <!-- EN: The global variable $root is in the file evt_builder-main.xsl-->
                  <!-- IT: La variabile globale $root si trova nel file evt_builder-main.xsl--><!-- Parameters -->
          ...
          --><xsl:param name="mainPrefix" select="'.'"/><xsl:param name="filePrefix" select="'../../..'"/><xsl:param name="dataPrefix" select="'../..'"/><xsl:param name="imageExt" select="'png'"/><!-- EN: Main web site for digital edition -->
        <!-- IT: Sito web principale dell'edizione digitale -->
        <!-- default: '' --><xsl:param name="webSite" select="'https://github.com/pax3l/m2_ovid_deities_analysis_for_quarto'"/><!-- EN: Edition title -->
        <!-- IT: Titolo edizione -->
        <!-- default: 'Codex Viewer' --><xsl:param name="index_title" select="'Les Métamorphoses d'Ovide — Apollon, Jupiter, Minerve'"/><!-- EN: Welcome Message -->
          <!-- IT: Messaggio di benvenuto -->
          <!-- default: 'Welcome to an edition created with EVT' --><xsl:param name="welcomeMsg"><div>
      <p class="title main">Bienvenue dans cette édition numérique des Métamorphoses créée avec EVT 1</p>
    </div>
    <div>Cette édition fait partie de mon mémoire de recherche de 2e année de Master Humanités classiques et huamnités numériques à l'Université Paris 10 Nanterre, sous la direction de monsieur Aurélien Berra.</div>
    <div>Pour plus d'informations sur l'utilisation de la présente édition, référez-vous au manuel dans le dossier "doc". 
Pour plus d'infirmations concernant le mémoire, vous pouvez consulter le dépôt en ligne <a href="https://github.com/pax3l/m2_ovid_deities_analysis_for_quarto>ici</a>.
Si vous avez des questions, des suggestions ou que vous repérez un bug, n'hésitez pas à laisser une issue sur <a href="">ce dépôt</a>.

For more information refer to the EVT Manual in the "doc" folder.
...
<div>Axelle Penture</div></xsl:param><!-- EN: Hide/Show scans -->
        <!-- IT: Nascondi/Mostra scansioni -->
        <!-- default: true() --><xsl:param name="image_frame" select="false()"/><!-- EN: Hide/Show badge -->
          <!-- IT: Nascondi/Mostra badge -->
          <!-- default: true() --><xsl:param name="badge" select="true()"/><!-- EN: Set text in badge -->
            <!-- IT: Imposta testo del badge -->
            <!-- ex: alpha, beta, stable etc --><xsl:param name="badge_text" select="'DIGITAL'"/><!-- EN: On/Off double page view -->
              ...
              <!-- default: false() --><xsl:param name="regesto" select="false()"/><!-- EN: On/Off <front> Information -->
          <!-- IT: Attiva/Disattiva Informazioni del <front> -->
          <!-- default: true() --><xsl:param name="frontInfo" select="true()"/><!-- EN: On/Off Manuscript Description -->
            <!-- IT: Attiva/Disattiva Descrizione del manoscritto-->
            <!-- default: true() --><xsl:param name="msDesc" select="true()"/><!-- EN: On/Off Header general information -->
              <!-- IT: Attiva/Disattiva Informazioni generali -->
              <!-- default: true() --><xsl:param name="headerInfo" select="true()"/><!-- EN: On/Off Bibliography -->
                <!-- IT: Attiva/Disattiva Bibliografia -->
                <!-- default: true() --><xsl:param name="bibliography" select="true()"/><!-- ############## -->
                  <!-- EDITION LEVELS -->
                  <!-- ############## -->
                  ...
                  <!-- IT: E' possibile rimuovere la produzione di pagine di una determinata edizione
                    semplicemente rimuovendo la parte testuale dell'item corrispondente. --><xsl:param name="edition_array" as="element()*"><edition>Diplomatic</edition><edition>Interpretative</edition><!-- EN: To add a new edition it is necessary to add a new line here and -forcedly- a
                ...
                Esempio: <edition>Nuova_edizione</edition> Add by FS --></xsl:param><!-- EN: It is possibile to customize the prefix used in the creation of the classes of the html elements of the edition -->
        <!-- IT: E' possibile personalizzare il prefisso usato nella creazione delle classi degli elementi html dell'edizione. --><xsl:variable name="ed_name1">dipl</xsl:variable><xsl:variable name="ed_name2">interp</xsl:variable><!-- EN: Enable/Disable translation -->
          <!-- IT: Attiva/Disattiva traduzione --><xsl:param name="translation" select="false()"/><!-- Thumbnail image --><xsl:param name="fb_thumb" select="''"/><!-- EN: Indicate the xml node that contains all the text to be transformed for each edition level -->
          ...
          <!--<xsl:variable name="ed_content" select="if(//tei:text/tei:group[@xml:id='group'])
           then(//tei:text/tei:group[@xml:id='group']/name()) else ( //tei:body/name() )"/>--><xsl:param name="ed_content" select="if(//tei:text/tei:group) then(//tei:text/tei:group/name()) else ( //tei:text/name() )"/><!-- EN: Starting point for the split of elements containing <pb/> and <lb/> elements -->
        ...
        <!-- Retrocompatiblità gestita solo per documenti unitari --><xsl:param name="start_split" select="if(//tei:text/tei:group) then(//tei:text/tei:group/name()) else(if(count(//tei:body/tei:div[@subtype='edition_text'])>1) then(//tei:body/tei:div[@subtype='edition_text']/name()) else(//tei:text/tei:body/name() ))"/><xsl:param name="defaulTextLabel" select="true()"/><!-- ################# -->
        <!-- INTERFACE CONTROL -->
        <!-- ################# -->
        ...
        --><xsl:param name="left_frame_default_content" select="'info'"/><!-- EN: Set default content on first load for right frame choosing between text or text front info
        Possible values are:
        ...
        --><xsl:param name="right_frame_default_content" select="'text'"/><!-- BUTTONS PRESENCE AND POSITION -->

          <!-- EN: Show/Hide Txt/Img Link Button in interface -->
          <!-- IT: Mostra/Nascondi pulsante Txt/Img Link nell'interfaccia web -->
          <!-- default: true() --><xsl:param name="txtimg_link_button" select="false()"/><!-- EN: Show/Hide Hotspot Button in interface -->
        ...
        <!-- default: true() --><xsl:param name="mag_button" select="false()"/><!-- EN: Show/Hide Thumbnails Button in interface -->
        <!-- IT: Mostra/Nascondi pulsante Thumbnails nell'interfaccia web -->
        <!-- default: true() --><xsl:param name="thumbs_button" select="true()"/><!-- EN: Show/Hide Viscoll Button in interface -->
          <!-- IT: Mostra/Nascondi pulsante Viscoll nell'interfaccia web -->
          <!-- default: false() --><xsl:param name="viscoll_button" select="false()"/><!-- VISCOLL -->
            ...
            <!-- IT: Percorso al file xml contenente lo schema viscoll.
              ...
              inserire qui solo percorso relativo a partire da quella cartella. --><xsl:param name="viscoll_scheme_path" select="''"/><!-- EN: Path to xml file containing viscoll images list.
              ...
              <!-- IT: Percorso al file xml contenente la lista delle immagini necessaria al corretto funzionamento di viscoll.
                ...
                e inserire qui solo percorso relativo a partire da quella cartella. --><xsl:param name="viscoll_image_list_path" select="''"/><!-- EN: Show/Hide Edition level selector in interface -->
        ...
        <!-- default: true() --><xsl:param name="edition_level_selector" select="true()"/><!-- IT: Choose page selector position -->
          ...
          <!-- "left" or "right" | Default: "right" --><xsl:param name="pp_selector_pos" select="'right'"/><!-- EN: Choose whether or not to group pages by document in the selector -->
            <!-- IT: Scegli se raggruppare o meno le pagine per documento nel selettore apposito -->
            <!-- default: true() --><xsl:param name="pp_selector_doc_grouping" select="true()"/><!-- EN: Choose whether or not having a tooltip on pages option showing the belonging document  -->
              <!-- IT: Scegli se avere un tooltip sulle opzioni delle pagine che mostra il/i documento/i di appartenenza -->
              <!-- default: false() --><xsl:param name="pp_selector_doc_tooltip" select="true()"/><!-- EN: On/Off Search -->
                <!-- IT: Attiva/Disattiva Ricerca -->
                <!-- default: true() --><xsl:param name="search" select="true()"/><!-- EN: On/Off Virtual Keyboard for search -->
                  <!-- IT: Attiva/Disattiva Tastiera virtuale per ricerca -->
                  <!-- default: true() --><xsl:param name="virtual_keyboard_search" select="false()"/><!-- EN: On/Off Bottom Navbar -->
                    <!-- IT: Attiva/Disattiva Barra di navigazione in fondo alla pagina -->
                    <!-- default: true() --><xsl:param name="bottom_navbar" select="false()"/><!-- EN: Initial status of Bottom Navbar (only works if $bottom_navbar=true()) -->
        <!-- IT: Stato iniziale della barra di navigazione (considerato solo se $bottom_navbar=true())-->
        <!-- possible values: 'collapsed' | 'expanded' -->
        <!-- default: 'collapsed' --><xsl:param name="bottom_navbar_initial_status" select="'collapsed'"/><!-- EN: On/Off Document Navigation -->
          <!-- IT: Attiva/Disattiva navigazione per documento -->
          <!-- default: false() --><xsl:param name="document_navigation" select="true()"/><!-- EN: On/Off Lines/Prose visualization Button -->
            <!-- IT: Attiva/Disattiva Bottone di visualizzazione Versi/Prosa  -->
            <!-- default: true() --><xsl:param name="prose_verses_toggler" select="true()"/><!-- EN: On/Off Tooltip indicating language encoded with @xml:lang -->
              <!-- IT: Attiva/Disattiva Tooltip per la visualizzazione della lingua degli elementi che presentano @xml:lang  -->
              <!-- default: false() --><xsl:param name="lang_tooltip" select="false()"/>
              <!-- LISTS -->

              <!-- EN: On/Off persons list -->
              <!-- IT: Attiva/disattiva lista persone -->
              <!-- default: true() --><xsl:param name="list_person" select="true()"/><!-- EN: On/Off places list -->
                <!-- IT: Attiva/disattiva lista luoghi -->
                <!-- default: true() --><xsl:param name="list_place" select="false()"/><!-- EN: On/Off orgName list -->
                  ...
                  <!-- default: true() --><xsl:param name="list_org" select="false()"/><!-- EN: On/Off term list -->
                    ...
                    <!-- default: true() --><xsl:param name="list_term" select="false()"/><!-- EN: On/Off gloss list -->
                      ...
                      <!-- default: true() --><xsl:param name="list_gloss" select="false()"/><!-- EN: On/Off chronological index for texts -->
                        <!-- IT: Attiva/disattiva indice cronologico -->
                        <!-- default: true() --><xsl:param name="list_doc" select="false()"/><!-- EN: It is possibile to personalize the elements in the filter
          ...
          html con il quale sono state marcate le parole "particolari" da selezionare. --><xsl:param name="lists" as="element()*"><group label="Entités nommées" active="true"><persName active="true" label="Divinités" ana="NA" type="narrative" ana="act" ana="auto" ana="obj" type="metamorphosis" ref="MIN"="" ref="APO"="" ref="IUP"=""/><placeName active="false"/><orgName active="false"/><sp  active="true" who="" ana="" corresp=""/></group><group label="Structure" active="true"><milestone active="true"/><<div> active="true" label="livre" type="textpart" subtype="book"=""/><date active="false"/><foreign active="false"/></group><group label="STAGES" active="false"><setting active="true"/><entrance active="true"/><exit active="true"/><business active="true"/><delivery active="true"/><modifier active="true"/><novelistic active="true"/><mixed active="true"/></group><group label="OTHER" active="false"><seg active="true" label="METAPHOR" type="metaphor"/></group></xsl:param><!-- EN: Information about EVT -->
            <!-- IT: Informazioni su EVT  --><xsl:param name="evtTxt"><p>EVT (Edition Visualization Technology) is a software for creating and browsing
    ...
    <a href="mailto:evt.developers@gmail.com">evt.developers@gmail.com</a></p></xsl:param></xsl:stylesheet>